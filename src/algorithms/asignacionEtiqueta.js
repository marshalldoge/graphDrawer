...
let permutations = []
export function asignacion(matrixad, task, labels){ 
    ...
    if(info.sources.length>=info.destinies.length){
        ...
        for(let i=0;i<info.destinies.length;i++){
            response.message = response.message.concat("El nodo ",labels[solution[i]]," es asignado a ",labels[info.destinies[i]],"\n");
            let object1 = {
                type: "node",
                id: solution[i],
                color: colors[i]
        ...
        for(let i=0;i<info.sources.length;i++){
            response.message = response.message.concat("El nodo ",labels[info.sources[i]]," es asignado a ",labels[solution[i]],"\n");
            let object1 = {
                type: "node",
                id: info.sources[i],
                color: colors[i]
